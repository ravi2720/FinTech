...
public partial class AEPSRegistration : System.Web.UI.Page
{
        ...
        if (Request.QueryString["loginid"] != null)
        {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["loginid"] != null)
                {
                    ...
                    //txtMemberID.Text = dtMemberMaster.Rows[0]["LoginID"].ToString();
                    txtMemberID.Text = LoginID;
        ...
        try
        {
            ...
            if (Checkvalid == 0)
            {
                using (var client = new WebClient())
                {
                    ...
                    //[{"Message":"Invalid Format Pan Number","StatusCode":"023"}]
                    //[{"emailid":"rubulsjobinfo@gmail.com","bc_id":"BC851775128","phone1":"7894603904","Message":"Success","Statuscode":"000"}]
                    ...
                    {
                        JObject ObjDataMain = JObject.Parse(responseString);
                        if (ObjDataMain["code"].ToString().ToLower() != "err")
                        {
                            JArray ObjData = JArray.Parse(ObjDataMain["data"].ToString());
                            if (ObjData[0]["StatusCode"] == null)
                            {
                                if (ObjData[0]["Statuscode"].ToString().ToUpper() == "000")
                                {
                                  ...
                                  //  ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Script", "alert('BC Registration Successfull.Your BC ID is : " + ObjData[0]["bc_id"].ToString() + "');", true);
                                    clear();
                                }
                                else
                                {
                                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Script", "alert('Some Error Occurred!! " + ObjData[0]["Message"].ToString() + "');", true);
                                }
                            }
                            else
                            {
                        ...
                        {
                            ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "Script", "alert('Some Error Occurred!! " + ObjDataMain["mess"].ToString() + "');", true);
        ...
        try
        {
            if (txtMemberID.Text != "")
            {
                ...
                else
                {
                    ...
                    dtMemberDetails = ObjAccess.select_data_dt("Select Name,Email,Mobile,StateID,pan from Member where loginid='" + txtMemberID.Text.Trim() + "'");
                    if (dtMemberDetails.Rows.Count > 0)
                    {
                        if(dtMemberDetails.Rows[0]["Name"].ToString().Split(' ').Length == 2)
                        {
                            txtFirstName.Text = dtMemberDetails.Rows[0]["Name"].ToString().Split(' ')[0];
                            //txtMiddleName.Text = dtMemberDetails.Rows[0]["MiddleName"].ToString();
                            txtLastName.Text = dtMemberDetails.Rows[0]["Name"].ToString().Split(' ')[1];
                        ...
                        dllState.SelectedValue = dtMemberDetails.Rows[0]["StateID"].ToString();
                        ...
                        dllState_SelectedIndexChanged(sender,e);
