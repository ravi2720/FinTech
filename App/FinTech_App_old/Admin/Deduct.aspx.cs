...
public partial class Admin_AddFund : System.Web.UI.Page
{
    ...
    protected void ddlMember_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        DataTable dtMember = cls.select_data_dt("select * from View_MemberBalance where msrno = '" + Convert.ToInt32(ddlMember.SelectedValue) + "'");
    ...
    {
        multiview1.ActiveViewIndex = 1;
        DataTable dtMember = cls.select_data_dt("select * from View_MemberBalance where msrno = '" + msrno + "'");
        rptMember.DataSource = dtMember;
        rptMember.DataBind();
    }
        ...
        try
        {
            if (!string.IsNullOrEmpty(txtAmount.Text) && Convert.ToDecimal(txtAmount.Text.Trim()) > 0 && !string.IsNullOrEmpty(txtTransactionPIN.Text))
            {
                if (txtTransactionPIN.Text == Session["OTPAddFund"].ToString())
                {
                    ...
                    string Narration = txtRemark.Text + "-" + Guid;
                    DataTable dtEwall = cls.select_data_dt("exec PROC_EWALLETTRANSACTION '" + lblLoginid.Text + "','" + txtAmount.Text + "','DR','" + Narration + "','" + txtRemark.Text + "','" + ConstantsData.SFundAdd + "'");
                    if (dtEwall.Rows.Count > 0)
                    {
                        if (dtEwall.Rows[0]["MSRNO"].ToString() == "1")
                        {
                            Int32 Msrno = cls.select_data_scalar_int("select msrno from member where loginid='" + lblLoginid.Text + "'");
                            Activity.AddActivity(dtEmployee.Rows[0]["ID"].ToString(), ConstantsData.SFundAdd.ToString(), Guid, $"{txtAmount.Text} Amount Deduct By { dtEmployee.Rows[0]["Name"].ToString()}", Msrno.ToString(), "Amount Deduct", cls);
                            ErrorShow.AlertMessageWithRedirect(page, "Balance Deduct Successfully ..!!", "Deduct.aspx", ConstantsData.CompanyName);
                            ConstantsData.Clear(page.Controls);
                        }
                        else
                        {
                            ErrorShow.AlertMessage(page, dtEwall.Rows[0]["STATUS"].ToString(), ConstantsData.CompanyName);
                        }
                    }
                }
                else
