...
public partial class Admin_DeductFund : System.Web.UI.Page
{
        ...
        try
        {
            if (!string.IsNullOrEmpty(txtAmount.Text) && Convert.ToDecimal(txtAmount.Text.Trim()) > 0 && !string.IsNullOrEmpty(txtTransactionPIN.Text))
            {
                if (txtTransactionPIN.Text == Session["OTPAddFund"].ToString())
                {
                    ...
                    DataTable dtEwall = cls.select_data_dt("exec PROC_EWALLETTRANSACTION '" + lblLoginid.Text + "','" + txtAmount.Text + "','CR','" + Narration + "','" + txtRemark.Text + "','" + ConstantsData.SFundAdd + "'");
                    if (dtEwall.Rows.Count > 0)
                    {
                        if (dtEwall.Rows[0]["MSRNO"].ToString() == "1")
                        {
                            ...
                            Activity.AddActivity(dtEmployee.Rows[0]["ID"].ToString(), ConstantsData.SFundAdd.ToString(), Guid, $"{txtAmount.Text} Amount Add By { dtEmployee.Rows[0]["Name"].ToString()}", Msrno.ToString(), "Amount Add", cls);
                            ErrorShow.AlertMessageWithRedirect(page, "Balance Added Successfully ..!!", "AddFund.aspx", ConstantsData.CompanyName);
