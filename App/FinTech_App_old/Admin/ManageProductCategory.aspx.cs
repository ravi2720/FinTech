...
public partial class Admin_ManageProductCategory : System.Web.UI.Page
{
    ...
    {
        rptData.DataSource = ObjConnection.select_data_dt("EXEC FillProductCategory @Action='All', @RoleID=0,@ID=0,@ParentID=0");
        rptData.DataBind();
    ...
    {
        DataTable dtMainMenu = ObjConnection.select_data_dt("EXEC FillProductCategory @Action='ParentMenu', @RoleID=0,@ID=0,@ParentID=0");
        Common.BindDropDown(ddlMainMenu, dtMainMenu, "Name", "ID");
        ddlMainMenu.Items.Insert(0, new ListItem("-Select Main Menu-", "0"));
    }
    private void fillSubMenu(int id)
    {
        DataTable dtSubMenu = ObjConnection.select_data_dt("EXEC FillProductCategory @Action='ParentMenu', @RoleID=0,@ID=0,@ParentID=" + id + "");
    ...
    private void fill_update_menu(int id)
    {
        ...
        dt = ObjConnection.select_data_dt("EXEC FillProductCategory @Action='selectedMenu', @RoleID=0,@ID=" + id + ",@ParentID=0");
    ...
    {
        Val = ObjConnection.update_data("ManageProductCategory @Action='IsActive',@ID=" + ID + "");
        if (Val > 0)
        {
            FillMenu();
        }
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        if (fileUpload.HasFile)
        {
            try
            {
                fileUpload.PostedFile.SaveAs(MapPath("./images/ProductCategory/") + gd);
                logo = gd.ToString();
            }
            catch (Exception ex)
            {
        ...
        DataTable dt = new DataTable();
        dt = ObjConnection.select_data_dt("EXEC AddEditProductCategory " + id + ",'" + ddlMenuType.SelectedValue + "','" + txtMenuName.Text.Trim() + "','" + txtMenuLink.Text + "','" + txtMenuIcon.Text.Trim() + "'," + Convert.ToInt32(ddlMenuLevel.SelectedValue) + "," + parentmenu + "," + Convert.ToInt32(txtMenuPosition.Text.Trim()) + ",'" + logo + "'");
