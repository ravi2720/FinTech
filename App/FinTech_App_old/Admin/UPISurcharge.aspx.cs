...
public partial class Admin_UPISurcharge : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionManager.IsSessionAdmin)
        {
            ...
            actionButtonPermission = ActionButtonPermission.GetPermission(ConstantsData.UPISurcharge, dtMember.Rows[0]["RoleID"].ToString());
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageUPISurcharge 'Get','" + 0 + "' ,'" + ddlPackage.SelectedValue + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (actionButtonPermission.Submit)
        {
            ...
            {
                #region [Insert]
                Int32 Val = objconnection.update_data("AddEditUPISurcharge '" + hdnRoleID.Value + "','" + ddlPackage.SelectedValue + "','" + txtStartVal.Text + "'," + txtEndVal.Text + "," + txtsurcharged.Text + ",'" + chkflatd.Checked + "','" + dtMember.Rows[0]["ID"].ToString() + "'");
                if (Val > 0)
                {
                    FillData();
        ...
        {
            ErrorShow.AlertMessage(page, "Not Permission For Add UPI Surcharge", ConstantsData.CompanyName);
        }
    }
    #endregion
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageUPISurcharge 'Get','" + ID + "'");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
    ...
    protected void rptAepsCommission_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Edit UPI Surcharge", ConstantsData.CompanyName);
            }
        }
        else if (e.CommandName == "Delete")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Delete UPI Surcharge", ConstantsData.CompanyName);
            }
        }
    }
    ...
    {
        int val = objconnection.update_data("ManageUPISurcharge 'IsDelete'," + id + "");
        if (val > 0)
        {
            FillData();
        }
