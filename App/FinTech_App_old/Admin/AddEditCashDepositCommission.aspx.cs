...
public partial class Admin_AddEditCashDepositCommission : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionManager.IsSessionAdmin)
        {
            ...
            actionButtonPermission = ActionButtonPermission.GetPermission(ConstantsData.CashDepositCommission, dtMember.Rows[0]["RoleID"].ToString());
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageCashDepositCommission 'Get','" + 0 + "' ,'" + ddlPackage.SelectedValue + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (actionButtonPermission.Submit)
        {
            ...
            {
                #region [Insert]
                Int32 Val = objconnection.update_data("AddEditCashDepositCommission '" + hdnRoleID.Value + "','" + ddlPackage.SelectedValue + "','" + txtStartVal.Text + "'," + txtEndVal.Text + "," + txtsurcharged.Text + ",'" + chkflatd.Checked + "','" + dtMember.Rows[0]["ID"].ToString() + "'");
                if (Val > 0)
                {
                    FillData();
        ...
        {
            ErrorShow.AlertMessage(page, "Not Permission For Add CashDeposit Commission", ConstantsData.CompanyName);
        }
    }
    #endregion
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageCashDepositCommission 'Get','" + ID + "'");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
    ...
    protected void rptAepsCommission_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Edit CashDeposit Commission", ConstantsData.CompanyName);
            }
        }
    ...
    {
        int val = objconnection.update_data("ManageCashDepositCommission 'IsDelete'," + id + "");
        if (val > 0)
        {
            FillData();
        }
