...
public partial class Admin_VirtualSurcharge : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionManager.IsSessionAdmin)
        {
            ...
            actionButtonPermission = ActionButtonPermission.GetPermission(ConstantsData.VirtualSurcharge, dtMember.Rows[0]["RoleID"].ToString());
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageVirtualSurcharge 'Get','" + 0 + "' ,'" + ddlPackage.SelectedValue + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (actionButtonPermission.Submit)
        {
            ...
            {
                #region [Insert]
                Int32 Val = objconnection.update_data("AddEditVirtualSurcharge '" + hdnRoleID.Value + "','" + ddlPackage.SelectedValue + "','" + txtStartVal.Text + "'," + txtEndVal.Text + "," + txtsurcharged.Text + ",'" + chkflatd.Checked + "','" + dtMember.Rows[0]["ID"].ToString() + "'");
                if (Val > 0)
                {
                    FillData();
        ...
        {
            ErrorShow.AlertMessage(page, "Not Permission For Add Virtual Surcharge", ConstantsData.CompanyName);
        }
    }
    #endregion
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageVirtualSurcharge 'Get','" + ID + "'");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
    ...
    protected void rptAepsCommission_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Edit Virtual Surcharge", ConstantsData.CompanyName);
            }
        }
        else if (e.CommandName == "Delete")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Delete Virtual Surcharge", ConstantsData.CompanyName);
            }
        }
    }
    ...
    {
        int val = objconnection.update_data("ManageVirtualSurcharge 'IsDelete'," + id + "");
        if (val > 0)
        {
            FillData();
        }
