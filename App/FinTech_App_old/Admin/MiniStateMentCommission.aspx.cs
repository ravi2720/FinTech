...
public partial class Admin_MiniStateMentCommission : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionManager.IsSessionAdmin)
        {
            ...
            actionButtonPermission = ActionButtonPermission.GetPermission(ConstantsData.MiniStatement, dtMember.Rows[0]["RoleID"].ToString());
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageMiniStateMentCommission 'Get','" + 0 + "' ,'" + ddlPackage.SelectedValue + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (actionButtonPermission.Submit)
        {
            ...
            {
                #region [Insert]
                Int32 Val = objconnection.update_data("AddEditMiniStateMentCommission '" + hdnRoleID.Value + "','" + ddlPackage.SelectedValue + "','" + txtStartVal.Text + "'," + txtEndVal.Text + "," + txtsurcharged.Text + ",'" + chkflatd.Checked + "','" + dtMember.Rows[0]["ID"].ToString() + "'");
                if (Val > 0)
                {
                    FillData();
        ...
        {
            ErrorShow.AlertMessage(page, "Not Permission For Add MiniStateMent Commission", ConstantsData.CompanyName);
        }
    }
    #endregion
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManageMiniStateMentCommission 'Get','" + ID + "'");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
    ...
    protected void rptAepsCommission_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Edit MiniStateMent Commission", ConstantsData.CompanyName);
            }
        }
    ...
    {
        int val = objconnection.update_data("ManageMiniStateMentCommission 'IsDelete'," + id + "");
        if (val > 0)
        {
            FillData();
        }
