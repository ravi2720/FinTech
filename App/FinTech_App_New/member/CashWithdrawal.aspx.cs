...
using System.Web.UI;
using System.Xml;
public partial class Member_CashWithdrawal : System.Web.UI.Page
{
    ...
    protected void btnCW_Click(object sender, EventArgs e)
    {
        if (!string.IsNullOrEmpty(formmobile.Text.Trim()) && !string.IsNullOrEmpty(aadhar_number.Text) && !string.IsNullOrEmpty(hdnCapture.Value))
        {
            ...
            if (dtValc.Rows.Count > 0)
            {
                ...
                if (isLoggedIn)
                {
                    Int32 Val = ObjConnection.update_data("Exec Proc_Add_edit_AEPS " + dtMember.Rows[0]["msrno"].ToString() + ", '" + RefID + "','" + txtAmount.Text + "', 'CW', " + nationalbank.SelectedValue + ",'" + (("xxxx-xxxx-" + aadhar)) + "', '" + formmobile.Text + "', 'Pending','','','','','App','','',''");
                    if (Val > 0)
                    {
                        InstantPayCWRoot cashWithdrawal = new InstantPayCWRoot();

                        string iPAddress = cls_myMember.GetIPAddress();
                        Location location = new Location();
                        location = location.GetLocationDataCurrent(iPAddress);

                        cashWithdrawal.amount = txtAmount.Text;
                        cashWithdrawal.bankiin = nationalbank.SelectedValue;
                        cashWithdrawal.mobile = formmobile.Text;
                        cashWithdrawal.externalRef = RefID;
                        cashWithdrawal.captureType = "FINGER";
                        cashWithdrawal.latitude = location.Latitude;
                        cashWithdrawal.longitude = location.Longitude;
                        ...
                        cashWithdrawal.biometricData = thumbRequest;

                        string ipAddress = cls_myMember.GetIPAddress();
                        string Result = NWSPay.CashWithdrawal(cashWithdrawal.GetJson(), dtValc.Rows[0]["OutLetID"].ToString());
                        ...
                        if (Data["statusCode"]?.ToString() == "TXN")
                        {
                            ...
                            if (Val > 0)
                            {
                                try
                                {
                                    ...
                                    Decs = $"AEPS Amount - {cashWithdrawal.amount}. Done With Aadhar No {("xxxx-xxxx-" + aadhar)}. TransID - {RefID}";
                                    ObjConnection.update_data("exec PROC_AEPSWALLETTRANSACTION '" + dtMember.Rows[0]["LoginID"].ToString() + "','" + cashWithdrawal.amount + "','CR','" + narration + "','" + Decs + "','" + ConstantsData.SAeps + "','" + RefID + "'");
                                    ObjConnection.update_data("Exec Proc_AEPS_DistributeCommission '" + dtMember.Rows[0]["msrno"].ToString() + "'," + cashWithdrawal.amount + ", '" + RefID + "', '" + ConstantsData._TDS + "','" + ConstantsData.SAEPSCommission + "','" + ConstantsData._TDSVal + "'");
                                }
                                catch (Exception ex)
                                {
