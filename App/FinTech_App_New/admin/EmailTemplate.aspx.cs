...
public partial class Admin_EmailTemplate : System.Web.UI.Page
{
    ...
    {
        dt = ObjConnection.select_data_dt("ManageMailTemplate 'GetByMsrno'," + company.MemberID + "");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        ...
        {
            Val = ObjConnection.update_data("AddEditMailTemplate " + hdnRoleID.Value + ",'" + dllSMSCategory.SelectedValue + "','" + txtTemplateName.Text + "','" + txtTemplateID.Text + "','" + dtMember.Rows[0]["ID"].ToString() + "','" + chkActive.Checked + "','" + company.MemberID + "'");
            if (Val > 0)
            {
    ...
    {
        dt = ObjConnection.select_data_dt("ManageMailTemplate 'GetAll'," + ID + "");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
    ...
    {
        Val = ObjConnection.update_data("ManageMailTemplate 'IsActive'," + ID + "");
        if (Val > 0)
        {
