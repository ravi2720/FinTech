...
public partial class Admin_AddAEPSFund : System.Web.UI.Page
{
        ...
        try
        {
            if (!string.IsNullOrEmpty(txtAmount.Text) && Convert.ToDecimal(txtAmount.Text.Trim()) > 0 && !string.IsNullOrEmpty(txtTransactionPIN.Text))
            {
                if (txtTransactionPIN.Text == Session["OTPAddFund"].ToString())
                {
                    ...
                    DataTable dtval = cls.select_data_dt("exec PROC_AEPSWALLETTRANSACTION '" + lblLoginid.Text + "','" + txtAmount.Text + "','CR','" + Narration + "','" + txtRemark.Text + "'," + ConstantsData.SFundAdd + ",'" + GUID + "'");
                    if (dtval.Rows.Count > 0)
                    {
                        if (dtval.Rows[0]["MSRNO"].ToString() == "1")
                        {
                            ...
                            Activity.AddActivity(dtEmployee.Rows[0]["ID"].ToString(), ConstantsData.SFundAdd.ToString(), GUID, $"AEPS {txtAmount.Text} Amount Add By { dtEmployee.Rows[0]["Name"].ToString()}", Msrno.ToString(), "AEPS Amount Add", cls);
                            ErrorShow.AlertMessageWithRedirect(page, "Balance Added Successfully ..!!", "AddAEPSFund.aspx", ConstantsData.CompanyName);
                        }
                        else
                        {
