...
public static class ErrorShow
{
   ...
   public static void Success(Page page,string Message)
    ...
    public static void AlertMessage(Page page, string message, string title)
    {
    ...
    public static void AlertMessageWithRedirect(Page page, string message, string redirectUrl, string title)
    {
        ScriptManager.RegisterClientScriptBlock(
            page,
            page.GetType(),
            "alertRedirect",
            $"alertify.alert('{message}', function () {{ window.open('{redirectUrl}', '_self'); }}).set({{title:'{title}'}});",
            true
        );
    }
