...
public partial class Admin_PayoutSurcharge : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (SessionManager.IsSessionAdmin)
        {
            ...
            actionButtonPermission = ActionButtonPermission.GetPermission(ConstantsData.PayoutSurcharge, dtMember.Rows[0]["RoleID"].ToString());
            company = Company.GetCompanyInfo();
            page = HttpContext.Current.CurrentHandler as Page;
            if (!IsPostBack)
            {
                FillData();
                dllPaymentType.DataSource = ConstantsData.PaymentType;
                dllPaymentType.DataBind();
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManagePayoutSurcharge 'Get','" + 0 + "' ,'" + ddlPackage.SelectedValue + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        if (actionButtonPermission.Submit)
        {
            ...
            {
                #region [Insert]
                Int32 Val = objconnection.update_data("AddEditPayoutSurcharge '" + hdnRoleID.Value + "','" + ddlPackage.SelectedValue + "','" + txtStartVal.Text + "'," + txtEndVal.Text + "," + txtsurcharged.Text + ",'" + chkflatd.Checked + "','" + dtMember.Rows[0]["ID"].ToString() + "','"+ dllPaymentType.SelectedValue + "'");
                if (Val > 0)
                {
                    FillData();
        ...
        {
            ErrorShow.AlertMessage(page, "Not Permission For Add Payout Surcharge", ConstantsData.CompanyName);
        }
    }
    #endregion
    ...
    {
        dt = objconnection.select_data_dt("EXEC ManagePayoutSurcharge 'Get','" + ID + "'");
        if (dt.Rows.Count > 0)
        {
            hdnRoleID.Value = ID;
            ...
            dllPaymentType.SelectedValue = dt.Rows[0]["PaymentType"].ToString();
    ...
    protected void rptAepsCommission_ItemCommand(object source, RepeaterCommandEventArgs e)
    {
        if (e.CommandName == "Edit")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Edit Payout Surcharge", ConstantsData.CompanyName);
            }
        }
        else if (e.CommandName == "Delete")
        {
            ...
            {
                ErrorShow.AlertMessage(page, "Not Permission For Delete Payout Surcharge", ConstantsData.CompanyName);
            }
        }
    }
    ...
    {
        int val = objconnection.update_data("ManagePayoutSurcharge 'IsDelete'," + id + "");
        if (val > 0)
        {
            FillData();
        }
