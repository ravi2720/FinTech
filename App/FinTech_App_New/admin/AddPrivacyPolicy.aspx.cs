...
public partial class Admin_AddPrivacyPolicy : System.Web.UI.Page
{
    ...
    protected void btnSubmit_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            DataTable dt = cls.select_data_dt("Exec AddEditPrivacyPolicy " + id + ",'" + txtNewsName.Text + "','" + ckNewsDesc.Text + "','" + dtMember.Rows[0]["ID"].ToString() + "','" + company.MemberID + "'");
    ...
    {
        dt = cls.select_data_dt("EXEC ManagePrivacyPolicy 'Get','" + 0 + "','" + company.MemberID + "'");
        if (dt.Rows.Count > 0)
        {
    ...
    private void FillNews(Int32 id)
    {
        dt = cls.select_data_dt("EXEC ManagePrivacyPolicy 'Get','" + id + "','" + company.MemberID + "'");
        txtNewsName.Text = dt.Rows[0]["Name"].ToString();
        ckNewsDesc.Text = dt.Rows[0]["Description"].ToString();
    }


    private void Active(Int32 id)
    {
        int val = cls.update_data("ManagePrivacyPolicy @Action='IsActive',@ID=" + id + ",'" + company.MemberID + "'");
        if (val > 0)
        {
            FillData();
        }
