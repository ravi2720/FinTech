...
{
    public class NSDLPANHistoryHandler : IRequestHandler<NSDLPANHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public NSDLPANHistoryHandler(ITransactionLogRepository transactionLogRepository,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(NSDLPANHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.NSDLPan).ToList();
