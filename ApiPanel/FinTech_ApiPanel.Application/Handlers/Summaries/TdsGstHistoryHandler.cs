...
{
    public class TdsGstHistoryHandler : IRequestHandler<TdsGstHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public TdsGstHistoryHandler(ITransactionLogRepository transactionLogRepository, IMapper mapper, ITokenService tokenService, IUserRepository userRepository)
        {
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(TdsGstHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.TDSGST).ToList();
