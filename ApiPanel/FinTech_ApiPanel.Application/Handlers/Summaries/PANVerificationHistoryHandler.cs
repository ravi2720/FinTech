...
{
    internal class PANVerificationHistoryHandler : IRequestHandler<PANVerificationHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public PANVerificationHistoryHandler(ITransactionLogRepository transactionLogRepository,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(PANVerificationHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.PanVerification).ToList();
