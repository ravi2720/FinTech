...
{
    internal class AllPendingHandler : IRequestHandler<AllPendingQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public AllPendingHandler(ITransactionLogRepository transactionLogRepository,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(AllPendingQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                walletTransactions = walletTransactions.Where(x => x.LogType.HasValue && GetIncludedLogTypes().Contains((byte)x.LogType) && x.Status == (byte)Status.Pending).ToList();
