...
{
    public class AadharVerificationHistoryHandler : IRequestHandler<AadharVerificationHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public AadharVerificationHistoryHandler(ITransactionLogRepository transactionLogRepository, IMapper mapper, ITokenService tokenService, IUserRepository userRepository)
        {
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(AadharVerificationHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.AadharVerification).ToList();
