...
{
    internal class UPICashOutHistoryHandler : IRequestHandler<UPICashOutHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public UPICashOutHistoryHandler(ITransactionLogRepository transactionLogRepository,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(UPICashOutHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.UTIPAN).ToList();
