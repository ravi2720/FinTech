...
{
    public class AepsPendingHistoryHandler : IRequestHandler<AepsPendingHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public AepsPendingHistoryHandler(IMapper mapper,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(AepsPendingHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                walletTransactions = walletTransactions
                    .Where(x => x.LogType.HasValue && GetIncludedLogTypes().Contains((byte)x.LogType) && x.Status == (byte)Status.Pending)
                    .ToList();
