...
{
    internal class DisputeHistoryHandler : IRequestHandler<DisputeHistoryQuery, ApiResponse<PagedResponse<TransactionLogDto>>>
    {
        ...
        public DisputeHistoryHandler(ITransactionLogRepository transactionLogRepository,
        ...
        public async Task<ApiResponse<PagedResponse<TransactionLogDto>>> Handle(DisputeHistoryQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                logs = logs.Where(x => x.LogType == (byte)LogType.Dispute).ToList();
