...
{
    public class RegisterBeneficiaryHandler : IRequestHandler<RegisterBeneficiaryCommand, object>
    {
        ...
        public RegisterBeneficiaryHandler(
        ...
        public async Task<object> Handle(RegisterBeneficiaryCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                var response = await PerformBeneficiaryRegistrationAsync(request, ip, outletId);

                var dto = JsonConvert.DeserializeObject<RegisterBeneficiaryDto>(response);
        ...
        private async Task<string> PerformBeneficiaryRegistrationAsync(RegisterBeneficiaryCommand command, string endpointIp, string outletId)
        {
            var url = baseUrl + InstantPayEndpoints.Remittance.DomesticV2.BeneficiaryRegistration;

            var requestBody = new
            {
                beneficiaryMobileNumber = command.BeneficiaryMobileNumber,
        ...
        private async Task LogTransaction(RegisterBeneficiaryDto dto, RegisterBeneficiaryCommand command, string endpointIp, string outletId, string userId, string response)
        {
            var log = new TransactionLog
            {
                ...
                Remark = dto.Status ?? "Beneficiary registration initiated",
