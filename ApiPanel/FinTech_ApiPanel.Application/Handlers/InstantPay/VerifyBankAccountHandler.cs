...
{
    public class VerifyBankAccountHandler : IRequestHandler<VerifyBankAccountCommand, object>
    {
        ...
        public VerifyBankAccountHandler(
        ...
        public async Task<object> Handle(VerifyBankAccountCommand request, CancellationToken cancellationToken)
        {
            try
            {
                //check  user wallet
                ...
                if (string.IsNullOrEmpty(endpointIp))
                    throw new Exception("Endpoint IP not found in context.");
                ...
                var response = await PerformBankVerificationAsync(request, ip, request.Latitude, request.Longitude);

                var dto = JsonConvert.DeserializeObject<VerifyBankAccountDto>(response);
        ...
        private async Task<string> PerformBankVerificationAsync(VerifyBankAccountCommand command, string endpointIp, string latitude, string longitude)
        {
            var url = baseUrl + InstantPayEndpoints.Identity.VerifyBankAccount;

            var payload = new
            {
                payee = new
                {
                    name = command.Payee.Name,
                    accountNumber = command.Payee.AccountNumber,
                    bankIfsc = command.Payee.BankIfsc
                },
                externalRef = command.ExternalRef,
                consent = command.Consent,
                pennyDrop = command.PennyDrop,
                latitude = latitude,
                longitude = longitude
            };
        ...
        private async Task LogTransaction(Wallet adminWallet, Wallet userWallet, VerifyBankAccountDto dto, VerifyBankAccountCommand command, string endpointIp, string outletId, string userId, string response, decimal userSurcharge)
        {
            var log = new TransactionLog
            {
                UserId = long.Parse(userId),
                LogType = (byte)LogType.BankAccountVerification_IPay,
                ...
                Ipay_Timestamp = DateTime.TryParse(dto.Timestamp, out var ts) ? ts : DateTime.UtcNow,
                ...
                Remark = dto.Status ?? "Bank verification completed",
                Ipay_Response = response,
            };

            var id = await _transactionLogRepository.AddAsync(log);

            if (string.Equals(dto.StatusCode, "TXN", StringComparison.OrdinalIgnoreCase))
                await LogTransactionForWallet(adminWallet, userWallet, dto, id, userSurcharge);
        }

        private async Task LogTransactionForWallet(Wallet adminWallet, Wallet userWallet, VerifyBankAccountDto dto, long logId, decimal userSurcharge)
        {
            try
            {
                ...
                await _transactionLogRepository.AddAsync(new TransactionLog
                {
                    ...
                    AuditType = (byte)AuditType.Debit,
                    LogType = (byte)LogType.Verify_BankAccount_Surcharge,
                    CreatedBy = adminWallet.UserId,
                    Status = (byte)Status.Success,
                    Remark = $"Admin Wallet Debit - Bank Account Verification Surcharge | Amount: {adminSurcharge} | For UserId: {userWallet.UserId} | Ref Txn: {logId}",
                    WalletUpdated = true,
                ...
                await _transactionLogRepository.AddAsync(new TransactionLog
                {
                    ...
                    Status = (byte)Status.Success,
                    Remark = $"User Wallet Debit - Bank Account Verification Surcharge | Amount: {userSurcharge} | Ref Txn: {logId}",
                    WalletUpdated = true,
                    ReferenceId = logId.ToString()
                });
                ...
                await _transactionLogRepository.AddAsync(new TransactionLog
                {
                    ...
                    CreatedBy = adminWallet.UserId,
                    Status = (byte)Status.Success,
                    Remark = $"Admin Wallet Credit - Bank Account Verification Surcharge Received from UserId: {userWallet.UserId} | Amount: {userSurcharge} | Ref Txn: {logId}",
                    WalletUpdated = true,
                    ReferenceId = logId.ToString()
                });
            ...
            try
            {
                ...
                if (userFinancialComponents != null && userFinancialComponents.Any())
                {
                    var surchargeSlab = userFinancialComponents
                        ...
                        x.ServiceSubType == (byte)FinancialComponenSubService.Verify_BankAccount);
