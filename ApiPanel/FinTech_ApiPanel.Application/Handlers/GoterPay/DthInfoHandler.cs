...
{
    public class DthInfoHandler : IRequestHandler<DthInfoQuery, object>
    {
        ...
        public DthInfoHandler(HttpClient httpClient,
        ...
        public async Task<object> Handle(DthInfoQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.Number))
                    throw new ArgumentException("DTH number is required");

                if (_isMaster)
                {
                    var responseContent = await GetDthInfoAsync(request);

                    var response = JsonConvert.DeserializeObject<DthInfoDto>(responseContent);

                    return response;
                }
                else
                {
                    var responseContent = await GetDthInfoAsync(request, outletId, endpointIp);
        ...
        private async Task<string> GetDthInfoAsync(DthInfoQuery request)
        {
            try
            {
                var url = $"https://api.goterpay.com/dthinfo" +
                ...
                var customerName = root.TryGetProperty("CustomerName", out var customerNameEl)
                    ? customerNameEl.GetString()
                    : "";

                var balance = root.TryGetProperty("Balance", out var balanceEl)
                    ? balanceEl.GetDecimal().ToString()  
                    : "0";

                var monthly = root.TryGetProperty("Monthly", out var monthlyEl)
                    ? monthlyEl.GetInt32().ToString()     
                    : "0";

                var nextRecharge = root.TryGetProperty("NextRecharge", out var nextRechargeEl)
                    ? nextRechargeEl.GetString()
                    : "";

                var planname = root.TryGetProperty("Planname", out var plannameEl)
                    ? plannameEl.GetString()
                    : "";
        ...
        private async Task<string> GetDthInfoAsync(DthInfoQuery query, string outletId, string endpointIp)
        {
            try
            {
                var clientConfig = await _clientRepository.GetAdminClientAsync();

                string url = $"{baseUrl}/Recharge/dth-info?operatorCode={query.OperatorCode}&number={query.Number}";
