...
{
    public class RechargePlanHandler : IRequestHandler<RechargePlanQuery, object>
    {
        ...
        public RechargePlanHandler(HttpClient httpClient,
        ...
        public async Task<object> Handle(RechargePlanQuery request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(request.Circle))
                    throw new ArgumentException("Circle code is required");

                if (_isMaster)
                {
                    var responseContent = await GetRechargePlansAsync(request);

                    var response = JsonConvert.DeserializeObject<RechargePlanDto>(responseContent);
                ...
                {
                    var responseContent = await GetRechargePlansAsync(request, outletId, endpointIp);
        ...
        private async Task<string> GetRechargePlansAsync(RechargePlanQuery request)
        {
            try
            {
                var url = $"https://api.goterpay.com/Rplan" +
                          ...
                          $"&circle={Uri.EscapeDataString(request.Circle)}";
                ...
                // Extract "Data" as raw JSON array
                ...
                var custom = new
                {
                    ...
                    data = dataElement, // inject the array directly
                    timestamp = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"),
        ...
        private async Task<string> GetRechargePlansAsync(RechargePlanQuery query, string outletId, string endpointIp)
        {
            try
            {
                var clientConfig = await _clientRepository.GetAdminClientAsync();

                string url = $"{baseUrl}/Recharge/plan?operatorCode={query.OperatorCode}&circle={query.Circle}";
