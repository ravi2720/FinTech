...
{
    public class UpdateWhitelistHandler : IRequestHandler<UpdateWhitelistCommand, ApiResponse>
    {
        private readonly IWhitelistRepository _whitelistRepository;
        private readonly IMapper _mapper;
        private readonly ITokenService _tokenService;
        public UpdateWhitelistHandler(IWhitelistRepository whitelistRepository,
        ...
        public async Task<ApiResponse> Handle(UpdateWhitelistCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (whitelistEntries.Any(x => x.Value == request.Value && x.Id != request.Id))
                    return ApiResponse<WhitelistDto>.ForbiddenResponse("Already exist");

                var whitelistInfo = whitelistEntries.Where(x => x.Id == request.Id).FirstOrDefault();
                ...
                _mapper.Map(request, whitelistInfo);
