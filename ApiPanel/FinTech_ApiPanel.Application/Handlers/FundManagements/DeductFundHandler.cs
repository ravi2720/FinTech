...
{
    internal class DeductFundHandler : IRequestHandler<DeductFundCommand, ApiResponse>
    {
        ...
        public DeductFundHandler(IFundRequestRepository fundRequestRepository,
        ...
        public async Task<ApiResponse> Handle(DeductFundCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (loggedInUser.UserId == request.UserId)
                    return ApiResponse.BadRequestResponse("You cannot deduct funds from your own wallet. Please contact support if you need assistance.");
                ...
                {
                    // manage admin and user wallets
                    await ManageAdminWallet(request, loggedInUser.UserId);
                    await ManageUserWallet(request);

                    scope.Complete();
                    return ApiResponse.SuccessResponse("Funds deducted successfully.");
                }
            }
            catch (Exception ex)
            {
        ...
        async Task ManageAdminWallet(DeductFundCommand request, long loggedInUserId)
        {
            try
            {
                // Load user's wallet
                ...
                var walletTransaction = new TransactionLog
                {
                    ...
                    Remark = !string.IsNullOrEmpty(request.Remark) ? request.Remark : $"Amount {request.Amount} deducted from user with id: {request.UserId} added to main wallet.",
                    LogType = (byte)LogType.DeductFund,
        ...
        async Task ManageUserWallet(DeductFundCommand request)
        {
            try
            {
                // Load user's wallet
                ...
                if (userWallet.TotalBalance < request.Amount)
                    throw new Exception("Insufficient user wallet balance to process this request.");
                ...
                var walletTransaction = new TransactionLog
                {
                    ...
                    ReferenceId = request.ReferenceId,
                    Remark = !string.IsNullOrEmpty(request.Remark) ? request.Remark : $"Amount deducted by admin: {request.Amount}",
                    LogType = (byte)LogType.DeductFund,
                    Status = (byte)Status.Success,
                    WalletUpdated = true
                };
