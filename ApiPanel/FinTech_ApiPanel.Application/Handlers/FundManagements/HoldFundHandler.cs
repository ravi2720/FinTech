...
{
    public class HoldFundHandler : IRequestHandler<HoldFundCommand, ApiResponse>
    {
        ...
        public HoldFundHandler(IFundRequestRepository fundRequestRepository,
        ...
        public async Task<ApiResponse> Handle(HoldFundCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (loggedInUser.UserId == request.UserId)
                    return ApiResponse.BadRequestResponse("You cannot hold funds for yourself. Please contact support if you need assistance.");
                ...
                using (var scope = new TransactionScope(TransactionScopeOption.Required, txOptions, TransactionScopeAsyncFlowOption.Enabled))
                {
                    ...
                    var transaction = new TransactionLog
                    {
                        ...
                        Remark = string.IsNullOrEmpty(request.Remark) ? request.Remark : $"Fund held of {request.Amount} by admin",
                        LogType = (byte)LogType.HoldFund,
                    };
                    await _transactionLogRepository.AddAsync(transaction);

                    // 5. Commit transaction
                    scope.Complete();
                    return ApiResponse.SuccessResponse("Funds held successfully.");
                }
            }
            catch (Exception ex)
            {
                return ApiResponse.BadRequestResponse("An error occurred while holding funds: " + ex.Message);
            }
        }
    }
}
